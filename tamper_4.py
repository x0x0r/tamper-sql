...
def tamper(payload, **kwargs):
    ...
    # Sisipkan komentar SQL acak setelah kata kunci umum
    ...
    for comment in comments:
        if comment not in payload:
            ...
            payload = payload.replace("HAVING", "HAVING" + comment)
            payload = payload.replace("GROUP BY", "GROUP BY" + comment)
            payload = payload.replace("ORDER BY", "ORDER BY" + comment)

    # Ganti fungsi delay dengan varian lain
